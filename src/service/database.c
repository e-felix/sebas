...
static int query_result_callback(void *NotUsed, int argc, char **argv,
                                 char **azColName) {
...
void database_close(Database *db) { free(db); }
...
int database_init(Database *db) {
  ...
  s_sqlite_run_query(sqlite_db, "SELECT * FROM col_test",
                     query_result_callback);
